...
void main(List<String> args) async {
  ...
  try {
    ...
    final countRes = await client.from('inventario').select('*', const FetchOptions(count: CountOption.exact, head: true));
    print('Total de filas en inventario: ${countRes.count}');
    ...
    print('Muestra de primeros 100 almacenes (unique): $almacenes');
    ...
    final resDesc = await client.from('inventario').select('Almacen').order('Almacen', ascending: false).limit(50);
    final almacenesDesc = (resDesc as List).map((e) => e['Almacen']).toSet();
    print('Muestra de últimos 50 almacenes (unique): $almacenesDesc');
